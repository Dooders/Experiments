...
class SimulationVisualizer:
    ...
    def _setup_stats_panel(self):
        ...
        ttk.Label(scrollable_frame, text="Basic Statistics", 
                 font=('Arial', 10, 'bold')).pack(anchor="w", pady=(5,0))
        for var in ['cycle', 'system_agents', 'individual_agents', 'total_resources']:
            ttk.Label(scrollable_frame, textvariable=self.stats_vars[var]).pack(anchor="w")

        ttk.Label(scrollable_frame, text="Population Dynamics", 
                 font=('Arial', 10, 'bold')).pack(anchor="w", pady=(10,0))
        for var in ['births', 'deaths', 'population_stability', 'avg_lifespan']:
            ttk.Label(scrollable_frame, textvariable=self.stats_vars[var]).pack(anchor="w")

        ttk.Label(scrollable_frame, text="Resource Metrics", 
                 font=('Arial', 10, 'bold')).pack(anchor="w", pady=(10,0))
        for var in ['avg_resources', 'resource_efficiency', 'resource_density']:
            ttk.Label(scrollable_frame, textvariable=self.stats_vars[var]).pack(anchor="w")

        ttk.Label(scrollable_frame, text="Territory Control", 
                 font=('Arial', 10, 'bold')).pack(anchor="w", pady=(10,0))
    ...
    def _setup_controls(self):
        ttk.Button(self.controls_frame, text="Pause/Resume", command=self._toggle_simulation).pack(side=tk.LEFT, padx=5)
        ttk.Button(self.controls_frame, text="Step Forward", command=self._step_forward).pack(side=tk.LEFT, padx=5)
        
        # Speed control with wider range
        ttk.Label(self.controls_frame, text="Simulation Speed:").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(self.controls_frame, text="Update Frequency:").pack(side=tk.LEFT, padx=5)
        self.freq_scale = ttk.Scale(self.controls_frame, from_=1, to=20, orient=tk.HORIZONTAL)
        self.freq_scale.set(5)  # Update every 5 steps by default
        self.freq_scale.pack(side=tk.LEFT, padx=5)
        
        # Save button
        ttk.Button(self.controls_frame, text="Save State", command=self._save_state).pack(side=tk.RIGHT, padx=5)
    ...
    def update(self, simulation_data):
        ...
        self.stats_vars['system_agents'].set(f"System Agents: {simulation_data['system_agents']}")
        self.stats_vars['individual_agents'].set(f"Individual Agents: {simulation_data['individual_agents']}")
        self.stats_vars['total_resources'].set(f"Total Resources: {simulation_data['total_resources']}")
        ...
        self.stats_vars['population_stability'].set(f"Population Stability: {simulation_data['population_stability']:.1%}")
        self.stats_vars['avg_lifespan'].set(f"Avg Lifespan: {simulation_data['average_lifespan']:.1f} cycles")
        self.stats_vars['avg_resources'].set(f"Avg Resources/Agent: {simulation_data['avg_resources']:.2f}")
        self.stats_vars['resource_efficiency'].set(f"Resource Efficiency: {simulation_data['resource_efficiency']:.1%}")
        self.stats_vars['resource_density'].set(f"Resource Density: {simulation_data['resource_density']:.2f}")
        self.stats_vars['system_territory'].set(f"System Territory: {simulation_data['system_agent_territory']:.1%}")
        self.stats_vars['individual_territory'].set(f"Individual Territory: {simulation_data['individual_agent_territory']:.1%}")
    ...
    def _update_chart(self, data):
        ...
        if len(history_df) > 0:  # Only plot if we have data
            ...
            self.ax1.plot(history_df.index, history_df['system_agent_count'], 'b-', label='System Agents')
            self.ax1.plot(history_df.index, history_df['individual_agent_count'], 'r-', label='Individual Agents')
            self.ax1.set_xlabel('Cycle')
            self.ax1.set_ylabel('Agent Count', color='b')
            
            # Plot resources on right axis
            self.ax2.plot(history_df.index, history_df['total_resources'], 'g-', label='Resources')
